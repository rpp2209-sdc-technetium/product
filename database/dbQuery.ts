...
const getStyle = (product_id, callback) => {
  ...
  db.pool.query(`
    SELECT
      ...
      styles."default?",
    (SELECT json_agg(json_build_object('thumbnail_url', thumbnail_url, 'url', url)::json)
      FROM photos
      WHERE photos.styleid = styles.style_id) AS photos,
    (SELECT json_object_agg(skus.id, json_build_object('size', skus.size, 'quantity', skus.quantity)::json)
      FROM skus
      WHERE skus.styleid = styles.style_id) AS skus
    FROM styles
